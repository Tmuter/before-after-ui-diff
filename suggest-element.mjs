 ...
 * stable hooks ([data-ui-diff] / id / [data-testid]) over a tag.class guess so
...
try {
  ...
  // a tiebreak. Build a stable selector preferring [data-ui-diff] → id →
  ...
  const candidates = await evalJs(`(() => {
    ...
    const sel = (el) => {
      if (el.dataset && el.dataset.uiDiff) return '[data-ui-diff="' + CSS.escape(el.dataset.uiDiff) + '"]';
